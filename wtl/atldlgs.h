...
template <class T>
class ATL_NO_VTABLE CShellFileDialogImpl : public IFileDialogEvents, public IFileDialogControlEvents
{
public:
// Operations
	INT_PTR DoModal(HWND hWndParent = ::GetActiveWindow())
...
// Implementation - IUnknown interface
	STDMETHOD(QueryInterface)(REFIID riid, void** ppvObject)
	{
		...
		if(IsEqualGUID(riid, IID_IFileDialogControlEvents))
		{
...
// Implementation - IFileDialogEvents interface
	...
	virtual HRESULT STDMETHODCALLTYPE IFileDialogControlEvents::OnItemSelected(IFileDialogCustomize* pfdc, DWORD dwIDCtl, DWORD dwIDItem)
	{
		T* pT = static_cast<T*>(this);
		ATLASSERT(pT->m_spFileDlg.IsEqualObject(pfdc));
		pfdc;   // avoid level 4 warning
		return pT->OnItemSelected(dwIDCtl, dwIDItem);
	}

    virtual HRESULT STDMETHODCALLTYPE IFileDialogControlEvents::OnButtonClicked(IFileDialogCustomize* pfdc, DWORD dwIDCtl)
	{
		T* pT = static_cast<T*>(this);
		ATLASSERT(pT->m_spFileDlg.IsEqualObject(pfdc));
		pfdc;   // avoid level 4 warning
    ...
    virtual HRESULT STDMETHODCALLTYPE IFileDialogControlEvents::OnCheckButtonToggled(IFileDialogCustomize* pfdc, DWORD dwIDCtl, BOOL bChecked)
	{
		T* pT = static_cast<T*>(this);
		ATLASSERT(pT->m_spFileDlg.IsEqualObject(pfdc));
		pfdc;   // avoid level 4 warning
    ...
    virtual HRESULT STDMETHODCALLTYPE IFileDialogControlEvents::OnControlActivating(IFileDialogCustomize* pfdc, DWORD dwIDCtl)
	{
		T* pT = static_cast<T*>(this);
		ATLASSERT(pT->m_spFileDlg.IsEqualObject(pfdc));
		pfdc;   // avoid level 4 warning
...
//#define PROP_LIST_CX		144
//#define IDW_PROPSHEET_SEARCH 0x7eee
//#define IDW_PROPSHEET_LIST	 0x7eef
//#define WM_PROP_SEL_CHANGE		(WM_APP+255)
//#define FOCUS_SEARCH_TIMER	10
//
//class CSearchBox : public CWindowImpl< CSearchBox >
//{
//public:
...
//	{
//		if( wParam == VK_DOWN ) {
//			HWND hwndSheet = GetParent();
//			ATLASSERT( hwndSheet );
//			::PostMessage( hwndSheet, WM_NEXTDLGCTL, (WPARAM)::GetDlgItem( hwndSheet, IDW_PROPSHEET_LIST ), TRUE );
//			return 0;
//		}
...
//};
//
//class CTitleList : public CWindowImpl< CTitleList >
//{
//public:
//	BEGIN_MSG_MAP( CTitleList )
//		MESSAGE_HANDLER( WM_KEYDOWN, OnKeyDown )
//	END_MSG_MAP()
//
//	LRESULT OnKeyDown( UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled )
...
//		HWND hwndSheet = GetParent();
//		ATLASSERT( hwndSheet );
//		int iSel = (int)SendMessage( LB_GETCURSEL, 0, 0 );
//		if( wParam == VK_UP && iSel == 0 ) {
//			::PostMessage( hwndSheet, WM_NEXTDLGCTL, (WPARAM)::GetDlgItem( hwndSheet, IDW_PROPSHEET_SEARCH ), TRUE );
//			return 0;
//		}
//		if( wParam == L'F' && GetKeyState( VK_CONTROL ) < 0 ) {
//			::SetTimer( hwndSheet, FOCUS_SEARCH_TIMER, 50, NULL );
//			return 0;
//		}
//		if( wParam == VK_RIGHT ) {
//			::PostMessage( hwndSheet, WM_NEXTDLGCTL, 0, FALSE );
//			return 0;
//		}
//		bHandled = FALSE;
//		return 0;
...
public:
	...
	//ATL::CSimpleArray<DLGTEMPLATE*> m_apDlgTemplate;
	//ATL::CSimpleArray<LPWSTR> m_asAdditionalText;
	//ATL::CSimpleArray<int> m_aiVisiblePage;
	...
	//void GetAccelString( ACCEL* pa, LPWSTR pBuf, int cchBuf )
	//{
	//	TCHAR sz[32];
	//	*pBuf = 0;
	//	if( pa->fVirt & FCONTROL ) {
	//		GetKeyName( sz, _countof( sz ), VK_CONTROL );
	//		StringCchCat( sz, _countof( sz ), _T( "+" ) );
	//		StringCchCopy( pBuf, cchBuf, sz );
	//	}
	//	GetKeyName( sz, _countof( sz ), pa->key );
	//	StringCchCat( pBuf, cchBuf, sz );
	//}
...
// Construction/Destruction
	...
	~CPropertySheetImpl()
	{
		...
		//DestroyGuiFont();
		//for( int i = 0; i < m_asAdditionalText.GetSize(); i++ ) {
		//	free( m_asAdditionalText[i] );
		//}
	}
...
// Callback function and overrideables
	static int CALLBACK PropSheetCallback(HWND hWnd, UINT uMsg, LPARAM lParam)
	{
		...
		//				pDlgTemplateEx->style |= WS_THICKFRAME | WS_VISIBLE;
		//			}
		//			else
		//			{
		...
		//				pDlgTemplate->style |= WS_THICKFRAME | WS_VISIBLE;
		//			}
		//		}
		//	}
		//}
	...
	//UINT GetScalePercent( HWND hwnd ) const
	//{
	//	_ASSERT( hwnd != NULL );
	...
	//		::SetErrorMode( n );
	//		if( hinst != NULL ) {
	//			UINT (WINAPI *lpGetDpiForSystem) () = NULL;
	//			lpGetDpiForSystem = (UINT (WINAPI*)())::GetProcAddress( hinst, "GetDpiForSystem" );
	//			if( lpGetDpiForSystem ){
	//				dpiSystem = lpGetDpiForSystem();
	//			}
	...
	//				dpiWindow = lpGetDpiForWindow( hwnd );
	//			}
	//			::FreeLibrary( hinst );
	//		}
	//	}

	//	if( dpiSystem != 0 && dpiWindow != 0 ) {
	//		return MulDiv( dpiWindow, 100, dpiSystem );
	//	}
	...
	//{
	//	if( !m_fontGUI ) {
	//		int nScalePercent = GetScalePercent( m_hWnd );
	//		if( nScalePercent != 100 || m_bListMode ) {
	//			NONCLIENTMETRICS ncm = { 0 };
	//			ncm.cbSize = sizeof(NONCLIENTMETRICS);
	//			if( SystemParametersInfo( SPI_GETNONCLIENTMETRICS, 0, &ncm, 0 ) ) {
	//				ncm.lfStatusFont;
	//				ncm.lfStatusFont.lfHeight = MulDiv( ncm.lfStatusFont.lfHeight, nScalePercent, 100 );
	//				m_fontGUI = CreateFontIndirect( &ncm.lfStatusFont );
	//			}
	//		}
	//	}
	//	return m_fontGUI;
	...
	//{
	//	if( m_fontGUI ) {
	//		DeleteObject( m_fontGUI );
	//		m_fontGUI = NULL;
	//	}
	//}

	//void MoveChildWindow( HWND hwnd, int dx, int dy )
	//{
	//	if( hwnd ) {
	//		RECT rc;
	...
	//int GetTabHeight()
	//{
	//	HWND hwndTab = PropSheet_GetTabControl( m_hWnd );
	//	_ASSERT( hwndTab );
	//	if( hwndTab == NULL )  return 0;
	//	RECT rcTab;
	//	::GetWindowRect( hwndTab, &rcTab );
	//	RECT rcTabInside = rcTab;
	//	TabCtrl_AdjustRect( hwndTab, FALSE, &rcTabInside );
	//	return rcTabInside.top - rcTab.top;
	//}

	//int GetSearchBoxHeight() const
	//{
	//	int nScalePercent = GetScalePercent( m_hWnd );
	//	RECT rcSearch = { 4, 4, 80, 16 };
	//	MapDialogRect( m_hWnd, &rcSearch );
	//	int cySearchBox = MulDiv( (rcSearch.bottom - rcSearch.top), nScalePercent, 100 );
	//	return cySearchBox;
	//}
	...
	void OnSheetInitialized()
	{
		...
		//	HWND hwndTab = PropSheet_GetTabControl( m_hWnd );
		//	if( hwndTab ) {
		//		DWORD dwStyles = (DWORD)::GetWindowLongPtr( hwndTab, GWL_STYLE );
		//		dwStyles &= ~TCS_MULTILINE;
		//		::SetWindowLongPtr( hwndTab, GWL_STYLE, dwStyles );

		//		::ShowWindow( hwndTab, SW_HIDE );
		//		::EnableWindow( hwndTab, FALSE );

		//		int cxListWidth = PROP_LIST_CX * GetScalePercent( m_hWnd ) / 100;
		//		int cyTabHeight = GetTabHeight();

		//		int cySearchBox = GetSearchBoxHeight();
		//		int cySpace = cySearchBox / 6;

		//		RECT rcSheet;

		//		// resize the property sheet
		//		GetWindowRect( &rcSheet );
		//		SetWindowPos( NULL, 0, 0, rcSheet.right - rcSheet.left + cxListWidth, rcSheet.bottom - rcSheet.top - cyTabHeight, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE );

		//		RECT rcTab;
		//		::GetWindowRect( hwndTab, &rcTab );
		//		::ScreenToClient( m_hWnd, (LPPOINT)&rcTab.left );
		//		::ScreenToClient( m_hWnd, (LPPOINT)&rcTab.right );
		//		// Move Tab control
		//		::SetWindowPos( hwndTab, NULL, rcTab.left + cxListWidth, rcTab.top - cyTabHeight, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE );

		//		// Move Cancel button
		//		HWND hwnd = ::GetDlgItem( m_hWnd, IDCANCEL );
		//		MoveChildWindow( hwnd, cxListWidth, 0 );
		...
		//		// Create Search box
		//		dwStyles = WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_TABSTOP | ES_AUTOHSCROLL;
		//		HWND hwndSearch = CreateWindowEx( WS_EX_CLIENTEDGE, L"edit", NULL, dwStyles, rcTab.left, rcTab.top, cxListWidth, cySearchBox, m_hWnd, (HMENU)IDW_PROPSHEET_SEARCH, NULL, NULL );
		//		SendMessage( hwndSearch, WM_SETFONT, (WPARAM)hFont, MAKELPARAM( FALSE, 0 ) );

		//		WCHAR szCueText[80];
		//		LoadString( m_psh.hInstance, IDS_SEARCH, szCueText, _countof( szCueText ) );
		//		ACCEL accel = { FCONTROL, 'F', 0 };
		//		StringCchCat( szCueText, _countof( szCueText ), L" (" );
		//		WCHAR sz[32];
		//		GetAccelString( &accel, sz, _countof( sz ) );
		//		StringCchCat( szCueText, _countof( szCueText ), sz );
		//		StringCchCat( szCueText, _countof( szCueText ), L")" );

		//		SendMessage( hwndSearch, EM_SETCUEBANNER, FALSE, (LPARAM)szCueText );
		//		m_SearchBox.SubclassWindow( hwndSearch );
		//		SendMessage( m_SearchBox.m_hWnd, EM_LIMITTEXT, 80, 0 );

		//		// Create Listbox
		//		dwStyles = WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_TABSTOP | LBS_NOTIFY | WS_HSCROLL | WS_VSCROLL/* | LBS_WANTKEYBOARDINPUT*/;
		//		HWND hwndList = CreateWindowEx( WS_EX_CLIENTEDGE, L"ListBox", NULL, dwStyles, rcTab.left, rcTab.top + cySearchBox + cySpace, cxListWidth, rcTab.bottom - rcTab.top - (cySearchBox + cySpace), m_hWnd, (HMENU)IDW_PROPSHEET_LIST, NULL, NULL );
		//		m_TitleList.SubclassWindow( hwndList );
		...
		//		hwnd = PropSheet_GetTabControl( m_hWnd );
		//		if( hwnd ) {
		...
		//			cx = MulDiv( cx, nScalePercent, 100 );
		//			cy = MulDiv( cy, nScalePercent, 100 );
		//			x = rc.right - cx;
		//			::SetWindowPos( hwnd, NULL, x, rc.top, cx, cy, SWP_NOZORDER | SWP_NOACTIVATE );
		//		}
		...
		//			int cyOrg = rc.bottom - rc.top;
		//			int cxDistance = xCancelLeft - rc.right;
		//			cxDistance = MulDiv( cxDistance, nScalePercent, 100 );
		...
		//		// resize the property sheet (y-direction)
		//		GetWindowRect( &rc );
		//		SetWindowPos( NULL, 0, 0, rc.right - rc.left, rc.bottom - rc.top + yInc, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE );
		//	}
		//}
	}
	...
	//		if( !m_bSecondShow ) {
	//			if( m_bListMode ) {
	//				HWND hwndList = ::GetDlgItem( m_hWnd, IDW_PROPSHEET_LIST );

	//				int cxListWidth = PROP_LIST_CX * GetScalePercent( m_hWnd ) / 100;
	//				int cyTabHeight = GetTabHeight();
	//				// Move Cancel button
	//				HWND hwnd = ::GetDlgItem( m_hWnd, IDCANCEL );
	//				MoveChildWindow( hwnd, cxListWidth, -cyTabHeight );
	...
	//				::ScreenToClient( hwnd, (LPPOINT)&rcButton.right );
	//				int yButtonBottom = rcButton.bottom;

	//				RECT rcList = { 0 };
	//				::GetWindowRect( hwndList, &rcList );
	//				::ScreenToClient( hwnd, (LPPOINT)&rcList.left );
	//				int yListTop = rcList.top;

	//				::SetWindowPos( hwndList, NULL, 0, 0, cxListWidth, yButtonBottom - yListTop, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE );

	//				::PostMessage( m_hWnd, WM_NEXTDLGCTL, (WPARAM)(HWND)GetDlgItem( IDW_PROPSHEET_LIST ), TRUE );
	//			}
	...
	//LRESULT OnPropSelChange( UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/ )
	//{
	//	if( m_bListMode ) {
	//		HWND hwndNewPage = (HWND)lParam;
	//		int iPage = PropSheet_HwndToIndex( m_hWnd, hwndNewPage );
	//		SelectListItem( iPage );
	//	}
	//	return 0;
	//}

	//void OnListItemChanged( WORD /*wNotifyCode*/, WORD /*wID*/, HWND hwndCtrl )
	//{
	//	int iPage = GetListCurItem();
	//	if( iPage >= 0 ) {
	//		PropSheet_SetCurSel( m_hWnd, NULL, iPage );
	//		::PostMessage( m_hWnd, WM_NEXTDLGCTL, (WPARAM)hwndCtrl, TRUE );
	//	}
	//}

	//BOOL GetPageTitle( int iPage, LPWSTR pBuf, int cchBuf )
	//{
	//	ATLASSERT( cchBuf > 0 );
	//	HWND hwndTab = PropSheet_GetTabControl( m_hWnd );
	...
	//	TCITEM ti = { 0 };
	//	ti.mask = TCIF_TEXT;
	//	ti.cchTextMax = cchBuf;
	//	ti.pszText = pBuf;
	//	return TabCtrl_GetItem( hwndTab, iPage, &ti );
	//}

	//void ResetList()
	//{
	//	ATLASSERT( m_apDlgTemplate.GetSize() == GetPageCount() );

	//	HWND hwndList = GetDlgItem( IDW_PROPSHEET_LIST );
	//	if( !hwndList )  return;
	...
	//	SIZE size;
	//	int cxMaxWidth = 0;
	//	int iOldPage = GetListCurItem();
	//	SendMessage( hwndList, WM_SETREDRAW, FALSE, 0 );
	//	SendMessage( hwndList, LB_RESETCONTENT, 0, 0 );
	//	m_aiVisiblePage.RemoveAll();

	//	TCHAR szFind[260];
	//	GetDlgItemText( IDW_PROPSHEET_SEARCH, szFind, _countof( szFind ) );

	//	for( int iPage = 0; iPage < m_apDlgTemplate.GetSize(); iPage++ ) {
	//		if( !szFind[0] || FindText( iPage, szFind ) ) {
	//			TCHAR szTitle[80];
	//			if( GetPageTitle( iPage, szTitle, _countof( szTitle ) ) ) {
	//				SendMessage( hwndList, LB_ADDSTRING, 0, (LPARAM)szTitle );
	//				::GetTextExtentPoint32( hdc, szTitle, lstrlen( szTitle ), &size );
	//				cxMaxWidth = max( cxMaxWidth, size.cx );
	//				m_aiVisiblePage.Add( iPage );
	//			}
	//		}
	//	}
	...
	//	::SendMessage( hwndList, LB_SETHORIZONTALEXTENT, cxMaxWidth + tm.tmAveCharWidth, 0 );

	//	if( iOldPage >= 0 ) {
	//		SelectListItem( iOldPage );
	//	}
	//	else {
	//		SelectCurrentPageListItem();
	//	}
	//	SendMessage( hwndList, WM_SETREDRAW, TRUE, 0 );
	//}

	//void OnSearchChange( WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hwndCtrl*/ )
	//{
	//	ResetList();
	//}
	...
	//		if( iSel >= 0 ) {
	//			if( iSel < m_aiVisiblePage.GetSize() ) {
	//				return m_aiVisiblePage[iSel];
	//			}
	//		}
	//	}
	//	return -1;
	...
	//void SelectListItem( int iPage )
	//{
	//	if( m_bListMode ) {
	//		HWND hwndList = GetDlgItem( IDW_PROPSHEET_LIST );
	//		if( hwndList ) {
	//			//ListView_SetItemState( hwndList, iPage, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED );
	//			int iSel = m_aiVisiblePage.Find( iPage );
	//			if( iSel >= 0 ) {
	//				SendMessage( hwndList, LB_SETCURSEL, (WPARAM)iSel, 0 );
	//			}
	//		}
	//	}
	//}

	//void SelectCurrentPageListItem()
	//{
	//	if( m_bListMode ) {
	//		SelectListItem( GetActiveIndex() );
	//	}
	//}
...
// Operations
	...
	//LPBYTE ScalePercentResource( HINSTANCE hInstance, LPCTSTR pszTemplateName, int nScalePercent )
	//{
	//	WORD wFontSize = 0;
	//	BYTE* pFontSizePos = NULL;
	//	LPBYTE pNew = NULL;

	//	if( hInstance ){
	//		HRSRC hRsrc = ::FindResource(hInstance, pszTemplateName, RT_DIALOG);  
	//		if( hRsrc ){
	...
	//					if (bDialogEx)
	//					{
	//						pw = (WORD*)((_DialogSplitHelper::DLGTEMPLATEEX*)pDlg + 1);
	//						dwStyle = ((_DialogSplitHelper::DLGTEMPLATEEX*)pDlg)->style;
	//					}
	//					else
	//					{
	//						pw = (WORD*)(pDlg + 1);
	...
	//													// Copy caption
	//						while (*pw++);				// Skip caption

	//						wFontSize = *pw;
	//						if( wFontSize != 0 ) {
	//							pFontSizePos = (LPBYTE)pw;
	//						}
	//					}

	//					pNew = new BYTE[dwSize];
	//					if( pNew ) {
	//						CopyMemory( pNew, pDlg, dwSize );
	//						if( wFontSize && pFontSizePos ) {
	//							*((WORD*)(pNew + (pFontSizePos - (LPBYTE)pDlg))) = (WORD)MulDiv( nScalePercent, (int)wFontSize, 100 );
	//						}
	//					}
	//				}
	...
	//BOOL AddPage(LPPROPSHEETPAGE pPage, LPCWSTR pszAdditionalText = NULL)
	//{
	//	ATLASSERT(pPage != NULL);
	//	if( pszAdditionalText == NULL ) {
	//		m_asAdditionalText.Add( NULL );
	//	}
	//	else {
	//		m_asAdditionalText.Add( _wcsdup( pszAdditionalText ) );
	//	}
	//	HPROPSHEETPAGE hPage = NULL;
	//	if( !(pPage->dwFlags & PSP_DLGINDIRECT) ) {
	//		int nScalePercent = GetScalePercent( m_psh.hwndParent );
	//		if( nScalePercent != 100 || m_bListMode ) {
	//			LPBYTE pNew = ScalePercentResource( pPage->hInstance, pPage->pszTemplate, nScalePercent );
	//			if( pNew ) {
	//				pPage->dwFlags |= PSP_DLGINDIRECT;
	//				pPage->pResource = (DLGTEMPLATE*)pNew;
	//				m_apDlgTemplate.Add( (DLGTEMPLATE*)pNew );
	...
	//bool FindMultiString( LPCWSTR pszText, LPCWSTR pszFind )
	//{
	//	if( lstrcmp( pszText, L"*" ) == 0 /*||  lstrcmp( pszText, L"Spin1" ) == 0 || lstrcmp( pszText, L"Slider1" ) == 0*/ ) {
	//		return false;
	//	}

	//	WCHAR sz[260];
	//	LPCWSTR pszSep = L" ,\t";
	//	LPWSTR pContext = NULL;
	//	StringCchCopyW( sz, _countof( sz ), pszFind );
	//	LPWSTR token = wcstok_s( sz, pszSep, &pContext );
	//	while( token != NULL ) {
	//		if( !StrStrI( pszText, token ) ) {
	//			return false;
	//		}
	//		token = wcstok_s( NULL, pszSep, &pContext );
	//	}
	//	return true;
	//}

	//BOOL FindText( int iPage, LPCWSTR pszFind )
	//{
	//	TCHAR szTitle[80];
	//	GetPageTitle( iPage, szTitle, _countof( szTitle ) );
	//	if( FindMultiString( szTitle, pszFind ) ) {
	//		return TRUE;
	//	}
	//	if( iPage < m_asAdditionalText.GetSize() ) {
	//		if( m_asAdditionalText[iPage] ) {
	//			if( FindMultiString( m_asAdditionalText[iPage] , pszFind ) ) {
	//				return TRUE;
	//			}
	//		}
	//	}

	//	DLGTEMPLATE* pDlg = m_apDlgTemplate[iPage];
	//	ATLASSERT( pDlg );
	//	if( pDlg ) {
	//		// Get first control on the template
	//		BOOL bDialogEx = _DialogSplitHelper::IsDialogEx( pDlg );
	...
	//			if( wID != IDOK && wID != IDCANCEL && wID != IDC_RESET && wID != IDC_SAMPLE && wID != IDC_SAMPLE_SCREEN && wID != IDC_SAMPLE_PRINT ) {
	//				WORD* pw;
	...
	//					if( *pw != 0 ) {
	//						if( FindMultiString( (LPCWSTR)pw, pszFind ) ) {
	//							return TRUE;
	//						}
	//					}
	//				}
	...
	//	return FALSE;
	//}
	// end of 2016-11-8  Customize by Yutaka
...
// Message map and handlers
	BEGIN_MSG_MAP(CPropertySheetImpl)
		//COMMAND_HANDLER_EX( IDW_PROPSHEET_LIST, LBN_SELCHANGE, OnListItemChanged )
		//COMMAND_HANDLER_EX( IDW_PROPSHEET_SEARCH, EN_CHANGE, OnSearchChange )
		//MESSAGE_HANDLER( WM_TIMER, OnTimer )
		MESSAGE_HANDLER(WM_COMMAND, OnCommand)
		MESSAGE_HANDLER(WM_SYSCOMMAND, OnSysCommand)
		//MSG_WM_SHOWWINDOW( OnShowWindow )
		//MESSAGE_HANDLER( WM_PROP_SEL_CHANGE, OnPropSelChange )
	END_MSG_MAP()

	//LRESULT OnTimer( UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled )
	//{
	//	if( wParam == FOCUS_SEARCH_TIMER ) {
	//		::KillTimer( m_hWnd, wParam );
	//		PostMessage( WM_NEXTDLGCTL, (WPARAM)(HWND)GetDlgItem( IDW_PROPSHEET_SEARCH ), TRUE );
	//		return 0;
	//	}
	//	bHandled = FALSE;
	//	return 0;
	...
	{
		//if( wParam == MAKEWPARAM( IDW_PROPSHEET_LIST, LBN_SELCHANGE ) ) {
		//	OnListItemChanged( LBN_SELCHANGE, IDW_PROPSHEET_LIST, (HWND)lParam );
		//}
		//if( wParam == IDOK ) {
		//	HWND hwndFocus = GetFocus();
		//	if( hwndFocus == GetDlgItem( IDW_PROPSHEET_SEARCH ) || hwndFocus == GetDlgItem( IDW_PROPSHEET_LIST ) ) {
		//		//::PostMessage( m_hWnd, WM_NEXTDLGCTL, (WPARAM)(HWND)GetDlgItem( IDW_PROPSHEET_LIST ), TRUE );
		//		::PostMessage( m_hWnd, WM_NEXTDLGCTL, 0, FALSE );
		//		return 0;
		//	}
		//	//if( GetFocus() == GetDlgItem( IDW_PROPSHEET_LIST ) ) {
		//	//	::PostMessage( m_hWnd, WM_NEXTDLGCTL, 0, FALSE );
		//	//	return 0;
		//	//}
		//}
		...
		//	HWND hwndFocus = GetFocus();
		//	if( hwndFocus == GetDlgItem( IDW_PROPSHEET_SEARCH ) ) {
		//		if( SendMessage( hwndFocus, WM_GETTEXTLENGTH , 0, 0 ) > 0 ) {
		//			SendMessage( hwndFocus, EM_SETSEL, 0, -1 );
		//			SendMessage( hwndFocus, WM_CLEAR, 0, 0 );
		//			return 0;
		//		}
		//	}
		//}
...
//bool CPropertySheetImpl<T, TBase>::m_bSetResizable = false;
...
{
public:
	CPropertySheet(ATL::_U_STRINGorID title = (LPCTSTR)NULL, UINT uStartPage = 0, HWND hWndParent = NULL/*, bool bResizable = false*/)
		: CPropertySheetImpl<CPropertySheet>(title, uStartPage, hWndParent/*, bResizable*/)
	{ }
};
...
// Construction
	...
	//		LPBYTE pNew = (LPBYTE)m_psp.pResource;
	//		delete [] pNew;
	//		m_psp.pResource = NULL;
	//	}
	//}
	//// end of 2016-11-8  Yutaka

	//virtual BOOL GetAdditionalText( LPWSTR /*pBuf*/, int /*cchBuf*/, int /*iCtrl*/ ) const
	//{
	//	return FALSE;
	//}

	//virtual LPCWSTR AdditionalText( LPWSTR pBuf, int cchBuf ) const
	//{
	//	ATLASSERT( cchBuf > 0 );
	//	*pBuf = 0;
	...
	//	TCHAR sz[520];
	//	for( int i = 0; ; i++ ) {
	//		if( !GetAdditionalText( sz, _countof( sz ), i ) ) {
	//			break;
	//		}
	//		if( *pBuf != 0 ) {
	//			StringCchCat( pBuf, cchBuf, L"::" );
	//		}
	//		if( FAILED( StringCchCat( pBuf, cchBuf, sz ) ) ) {
	//			ATLASSERT( FALSE );
	//			return (LPCWSTR)pBuf;
	//		}
	//	}
	//	return *pBuf == 0 ? NULL : (LPCWSTR)pBuf;
	//}
...
// Operations
	...
	//{
	//	HWND hwndTab = PropSheet_GetTabControl( GetParent() );
	//	return !::IsWindowVisible( hwndTab );
	//}
...
// Overridables
	...
	int OnSetActive()
	{
		...
		//SendMessage( GetParent(), WM_PROP_SEL_CHANGE, 0, (LPARAM)m_hWnd );
		return 0;
	}
	...
	{
		//if( lpMsg->message == WM_KEYDOWN && lpMsg->wParam == L'F') {
		//	bool bCtrl = GetKeyState( VK_CONTROL ) < 0;
		//	if( bCtrl ) {
		//		if( IsListMode() ) {
		//			::PostMessage( GetParent(), WM_NEXTDLGCTL, (WPARAM)(HWND)::GetDlgItem( GetParent(), IDW_PROPSHEET_SEARCH ), TRUE );
